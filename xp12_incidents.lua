...
local function smoke_fixable()
    if not _ref_bat_on or not _ref_avion or not _ref_gen_on then return false end
    local v = {}
    XPLMGetDatavi(_ref_gen_on, v, 0, 1)
    return XPLMGetDatai(_ref_bat_on) == 0
       and XPLMGetDatai(_ref_avion)  == 0
       and (v[1] or 1) == 0
end
...
def("SMOKE",          "sim/operation/failures/rel_smoke_cpit",       "Smoke",        "engine_or_elec", true, { cond = smoke_fixable })
...
def("ELV_TRIM_RUN",   "sim/operation/failures/rel_elv_trim_run",      "Elv Trim Run", nil, nil, { cond = trim_fixable, followup = { "TRIM_ELV", "TRIM_AIL", "TRIM_RUD" } })
def("AIL_TRIM_RUN",   "sim/operation/failures/rel_ail_trim_run",      "Ail Trim Run", nil, nil, { cond = trim_fixable, followup = { "TRIM_ELV", "TRIM_AIL", "TRIM_RUD" } })
def("RUD_TRIM_RUN",   "sim/operation/failures/rel_rud_trim_run",      "Rud Trim Run", nil, nil, { cond = trim_fixable, followup = { "TRIM_ELV", "TRIM_AIL", "TRIM_RUD" } })
...
function incidents_fix_tick()
    ...
    if cfg.mode == "OFF" then return end
    for _, f in ipairs(failures) do
        if f.fix and get_dr(f) == 6 and f.fix.cond() then
            reset_failure(f)
